...
def convert_markdown_links_to_wikilinks(file_path: str, dry_run: bool = False) -> None:
    ...
    for match in matches:
        ...
        if link_target.endswith(".md"):
            ...
            if (link_target.startswith("/") and not os.path.exists(link_target)) or (
                not os.path.exists(
                    os.path.join(os.path.dirname(file_path), link_target)
                )
            ):
                print(f"WARN: the file {link_target} does not exist locally. Skipping")
                continue
    ...
    if dry_run:
        ...
        with open(file_path, "w", encoding="utf-8") as file:
            file.write(content)
        print(f"Markdown links in {file_path} have been converted to Wikilinks.")
